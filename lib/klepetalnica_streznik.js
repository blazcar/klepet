...
function obdelajDregljaj(socket) {
  socket.on('dregljaj', function (dregljaj) {
    if (dregljaj.vzdevek) {
      var socketIdNaslovnika = null;
      for (var id in vzdevkiGledeNaSocket) {
        //console.log(vzdevkiGledeNaSocket[id]);
        //console.log(dregljaj.vzdevek + " " + vzdevkiGledeNaSocket[id]);
      ...
      if (socketIdNaslovnika) {
       if(socketIdNaslovnika == socket.id){
         io.sockets.sockets[socket.id].emit('sporocilo', {besedilo: "Dregljaja samemu sebi ni mogoče poslati!"});
       }
       else {
       ...
       io.sockets.sockets[socket.id].emit('sporocilo', {
         besedilo: "Dregljaja uporabniku z vzdevkom '" + dregljaj.vzdevek + "' ni bilo moč posredovati."
       });
      }
    }
  });
